...
> **Entry point:** `cmd/server.go` → `download/download.go`

```
Người dùng điền Form UI và submit:
POST /api/backtest
{ "pairs": "BTCUSDT", "timeframe": "1h", "days": 30 }
          │
          ▼
┌─────────────────────────────────────────────┐
│  cmd/server.go                              │
...
│    exchange.NewBinance()        (public API)│
└──────────────────┬──────────────────────────┘
                   │
                   ▼
┌─────────────────────────────────────────────┐
...
> **Entry point:** `cmd/server.go` → `runBacktest()`
...
runBacktest()
  ...
  │
  ├─2─► Khởi tạo Strategy theo form input (e.g. CrossEMA)
  │       strat = strategies.NewCrossEMA(...)
  │
...
> **Entry point:** `cmd/main.go` → HTTP server → Browser

```
main()
  │
  │  cmd/main.go & plot/chart.go
  ▼
┌──────────────────────────────────────────────────────────────┐
│  Đăng ký HTTP routes:                                        │
│                                                              │
│  GET /               → handleRoot()          (form/chart UI)│
│  POST /api/backtest  → handleBacktest()      (run bot)      │
│  GET /api/summary    → handleSummary()       (JSON summary) │
│  GET /data?pair=...  → handleData()          (JSON API)     │
│  GET /history?pair=. → handleTradingHistoryData() (CSV)     │
│  GET /assets/         → serve UI CSS/JS       (go:embed)     │
│                                                              │
...
└──────────────────────────┬───────────────────────────────────┘
         ...
         Browser mở http://localhost:8080
          ...
          │                                               │
          ▼                                               ▼
  GET /?pair=BTCUSDT                        GET /data?pair=BTCUSDT
          │                                               │
          │  Execute chart.html template                  │  handleData()
          │  → render dropdown chọn pair                  │
          │  → load JS/CSS assets                         │  Tổng hợp dữ liệu:
          │                                               │  ┌──────────────────────────────┐
          │                                               │  │ candlesByPair(pair)          │
          ▼                                               │  │  → gán orders vào đúng nến   │
    chart.js / chart_summary.js                           │  │                              │
    → fetch("/data?pair=BTCUSDT")  ──────────────────────►  │ indicatorsByPair(pair)        │
    → fetch("/api/summary")                               │  │  → chạy lại Indicators()     │
    → nhận JSON response                                  │  │  → slice bỏ warmup period    │
                                                          │  │                              │
...
model.Candle  (mỗi candle được xử lý tuần tự)
      ...
      └──► chart.OnCandle(candle)
                 ...
                 → chart.lastUpdate = now
                           ...
                           ▼
                    server.runBacktest() trả về {"pairs": ["BTCUSDT"]}
                           │
                    Browser redirect tới /?pair=BTCUSDT
                           │
                    Browser ← GET /data → JSON
                           │
                    Chart.js & Chart_summary.js render UI
```
...
|---|---|---|
| Web API download | [cmd/server.go](../cmd/server.go) | `runBacktest()` |
...
| HTTP server | [cmd/main.go](../cmd/main.go) | `main()` |

---

## Thứ tự gọi hàm khi User chạy Backtest (tóm tắt)

```go
// (Bên trong cmd/server.go -> runBacktest)

// 1. Chuẩn bị dữ liệu nguồn (tải từ public API)
...
// 3. Reset và config lại chart cho session mới
s.chart.Reset()
s.chart.SetStrategy(strat)
s.chart.SetPaperWallet(wallet)

// 4. Khởi tạo và kết nối tất cả
bot, _ := ninjabot.NewBot(ctx, settings, wallet, strat,
    ninjabot.WithBacktest(wallet),          // bật chế độ backtest
    ninjabot.WithStorage(storage),          // lưu orders vào memory DB
    ninjabot.WithCandleSubscription(s.chart), // feed nến vào chart
    ninjabot.WithOrderSubscription(s.chart),  // feed orders vào chart
)
...
// 7. Render JSON Summary & trả HTTP Response
summary := computeSummary(...)
s.summaryJSON = summary
return backtestResponse{Pairs: pairs}
```
